...
class EditPage(ctk.CTkFrame):
    ...
    def _get_edit_icon(self, name, size=(18, 18)):
        """Charge l'icône et force l'adaptation au mode sombre si nécessaire"""
        try:
            base_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
            icon_path = os.path.join(base_path, "assets", "edit", f"{name}.png")
            if os.path.exists(icon_path):
                img = Image.open(icon_path)
                # On utilise la même image, mais CTK gérera le contraste du bouton
    ...
    def _show_page(self):
        ...
        self.canvas.delete("pdf_bg") # Efface seulement le fond
    ...
    def _on_click(self, event):
        ...
        if self.tool_mode == "select":
            ...
            if "editable" in tags:
                ...
                self.canvas.itemconfig("editable", fill=self.current_color)
    ...
    def _on_drag(self, event):
        if self.tool_mode == "select" and self.drag_data["item"]:
            ...
            self.canvas.create_line(self.last_x, self.last_y, event.x, event.y, 
                                   fill=self.current_color, width=2, capstyle="round", tags=("editable", "draw_line"))
            self.last_x, self.last_y = event.x, event.y
    ...
    def save_changes(self):
        # Logique de sauvegarde à implémenter selon pdf_engine
        pass