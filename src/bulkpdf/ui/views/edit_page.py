...
def resource_path(relative_path):
    try:
        ...
        # Remonte au dossier racine du projet (bulkpdf/)
...
class EditPage(ctk.CTkFrame):
    ...
    def _get_edit_icon(self, name, size=(18, 18)):
        ...
        try:
            # Test de plusieurs chemins possibles pour les assets
            paths = [
    ...
    def _on_drag(self, event):
        if self.tool_mode == "select" and self.selected_items:
            cx, cy = self.canvas.canvasx(event.x), self.canvas.canvasy(event.y)
    ...
    def _rotate_selected(self):
        for item in self.selected_items:
            item_id = self._get_id(item)
            if "signature" in self.canvas.gettags(item_id) and item_id in self.image_refs:
                data = self.image_refs[item_id]
